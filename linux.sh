...
if grep -Eq "localhost|127\.0\.0\.1" .env 2>/dev/null; then
  if ! curl -fsS "http://localhost/v1/health/version" >/dev/null 2>&1; then
    ...
    elif [[ -f "/var/lib/docker/appwrite/docker-compose.yml" ]]; then
      docker compose --project-directory "/var/lib/docker/appwrite" -f "/var/lib/docker/appwrite/docker-compose.yml" up -d 2>/dev/null || \
      docker-compose -f "/var/lib/docker/appwrite/docker-compose.yml" up -d 2>/dev/null || true
    else
