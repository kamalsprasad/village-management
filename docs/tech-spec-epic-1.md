...
| `village_settings`           | `$id`, `village_name`, `address`, `established_date`, `default_currency`, `currency_symbol`, `timezone`, `country_code`, `is_using_sample_data`, `council_members[]`, `modules_enabled[]`, `$updatedAt` | Single, strongly typed row stored under Appwrite row ID `settings_root`                                                      | Drives banner, navigation toggles, and report headers; preserves validation/enum guardrails @docs/epics.md#188-205 |
...
- If future epics require experimental toggles, layer a secondary key-value table (`settings_flags`) while keeping `village_settings` authoritative for core metadata.
