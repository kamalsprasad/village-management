...
- source_spec: `docs/implementation-artifacts/spec-5-10b-navigation-polish-breadcrumbs-and-quick-search.md`
  ...
  evidence: `EnrollLearnerPage.vue`, `CreateInterventionPage.vue`, and several detail pages previously had destination-specific tooltips on their Back buttons; `Breadcrumbs.vue`'s mobile `q-btn` only has a generic `aria-label="Back"`. Low-severity accessibility regression, not required by Story 5.10b's ACs. Owning story: 5.10e1 (UX Polish and Accessibility — AC4 accessibility audit / aria-labels; per user decision 2026-08-11).
- source_spec: `docs/implementation-artifacts/spec-5-10b-navigation-polish-breadcrumbs-and-quick-search.md`
  ...
  evidence: `src/layouts/MainLayout.vue` `<q-menu ... no-focus no-refocus>` on the search results menu. Not required by Story 5.10b's ACs; tradeoff accepted to keep typing fluent. Owning story: 5.10e1 (UX Polish and Accessibility — AC4 accessibility audit; verify/restore keyboard reachability of search results without re-introducing the focus-steal regression; per user decision 2026-08-11).
...
- source_spec: `docs/implementation-artifacts/spec-5-10c-notifications-system.md`
  ...
  evidence: `src/layouts/MainLayout.vue` `onMounted` try/catch around `realtime.subscribe(...)`. This mirrors the exact same pattern already used by `src/modules/finance/composables/useDashboardData.js`'s `subscribeToRealtime` (the only other realtime consumer in the codebase), so it is a pre-existing systemic gap in the realtime pattern, not something introduced fresh by this story. A full fix (e.g. Appwrite `Realtime` connection-state/error callbacks, if the installed SDK version exposes them) should be applied to both consumers together. Owning story: post-MVP realtime-hardening pass (re-deferred from 5.10e per user decision 2026-08-11 — should fix both `MainLayout.vue` and `useDashboardData.js` realtime consumers together) — not required by Story 5.10c's ACs.
- source_spec: `docs/implementation-artifacts/spec-5-10c-notifications-system.md`
  ...
  evidence: `src/modules/school/stores/at-risk-store.js` `CACHE_TTL_MS = 60_000` (pre-existing) interacting with the new `notifyNewAtRiskLearners` call site. Low-impact (a 60-second window on a computation that is itself only refreshed on-demand per page view, not on a tight loop); not required by Story 5.10c's ACs, which only require detecting "newly at-risk" once per dedup key. Owning story: post-MVP at-risk-notification refinement (re-deferred from 5.10e per user decision 2026-08-11 — if this proves material in practice) — not required by Story 5.10c's ACs.

## Deferred from: Epic 5 planning — 5.10e split and post-MVP testing epic (user decision 2026-08-11)

- **5.10e split into 5.10e1–5.10e4.** Story 5.10e (UX Polish, Performance, Mobile Responsiveness, and Final Testing Checklist — AC4, AC5, AC6, AC9) covers 4 broad ACs spanning independently shippable concerns, the same multi-goal pattern that caused 5.10 to be split into 5.10a–5.10e. Per the bmad-dev-auto discipline, 5.10e is pre-split into: 5.10e1 (AC4 UX polish & accessibility), 5.10e2 (AC5 performance), 5.10e3 (AC6 mobile responsiveness — full 320px audit + all fixes), 5.10e4 (AC9 final testing checklist — manual doc). Order: e1 → e2 → e3 → e4. Completing 5.10e4 marks 5.10 and Epic 5 as done. `sprint-status.yaml` updated with `5-10e1` through `5-10e4` entries (all `backlog`).
