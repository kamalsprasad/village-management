...
- **Key Tasks:** Bulk record test scores/attendance, review at-risk dashboard, assign interventions, conduct teacher evaluations _(post-MVP)_
...
│   │   └── Reports
│   │
│   ├── Guests (if enabled) *(post-MVP — Story 5.5 deferred)*
│   │   ├── Guests List
│   │   ├── Add Guest
│   │   ├── Guest Detail
│   │   └── Conversion Alerts
│   │
│   ├── Equipment (if enabled) *(post-MVP — Story 5.6 deferred)*
│   │   ├── Equipment List
│   │   ├── Add Equipment
│   │   ├── Equipment Detail
...
│   │   └── Vendor Detail
│   │
│   └── Energy (if enabled) *(post-MVP — Story 5.8 deferred)*
│       ├── Energy Dashboard
...
- **Optional Modules** (only if enabled, grouped separator)
  ...
  - Guests (bed icon) _(post-MVP — Story 5.5 deferred)_
  - Equipment (tools icon) _(post-MVP — Story 5.6 deferred)_
  - Vendors (handshake icon)
  - Energy (lightning icon) _(post-MVP — Story 5.8 deferred)_
- Reports (chart icon)
- Settings (gear icon)
...
- **Teacher Evaluations:** Pending reviews count _(post-MVP)_
...
**Phase 4: Implementation (After Gate Check)** 3. **Story 1.1: Project Setup** - Initialize Quasar + Appwrite project with this UX spec as reference 4. **Implement Design System** - Configure Quasar theme with colors, typography, spacing from Section 5 5. **Build Core Components** - Create domain-specific components (PlotCard, LearnerCard, etc.) from Section 4.2 6. **Develop First Epic** - Implement Epic 1 stories following user flows from Section 3
