...
- [ ] **Task 1: Seed Roles Table** (AC: 1)
  - [ ] Subtask 1.1: Create roles table in Appwrite if not exists (may already exist from Story 1.2)
  - [ ] Subtask 1.2: Define role schema: id, name, permissions[], storage_quota, description, created_at
  - [ ] Subtask 1.3: Seed System Administrator role (permissions: ['*'], storage_quota: -1 for unlimited)
  - [ ] Subtask 1.4: Seed Village Head role (permissions: ['residents:read', 'residents:write', 'households:read', 'households:write', 'finance:read', 'dashboard:read'], storage_quota: 20GB)
  - [ ] Subtask 1.5: Seed Finance Manager role (permissions: ['finance:read', 'finance:write', 'inventory:read', 'dashboard:read'], storage_quota: 5GB)
  - [ ] Subtask 1.6: Seed Resident role (permissions: ['dashboard:read', 'calendar:read'], storage_quota: 2GB)
  - [ ] Subtask 1.7: Seed Guest role (permissions: ['calendar:read'], storage_quota: 1GB)
  - [ ] Subtask 1.8: Verify all 5 roles created successfully in Appwrite console

- [ ] **Task 2: Update User Profile Schema** (AC: 2)
  - [ ] Subtask 2.1: Add role_ids[] field to users table (string array type)
  - [ ] Subtask 2.2: Update auth-store to include role_ids in user state
  - [ ] Subtask 2.3: Fetch user roles on login and store in Pinia auth-store
  - [ ] Subtask 2.4: Update user profile page to display assigned roles
  - [ ] Subtask 2.5: Test multi-role assignment by manually assigning multiple roles via Appwrite console

- [ ] **Task 3: Create Permission Checking Utility** (AC: 3, 4)
  - [ ] Subtask 3.1: Create src/utils/permissions.js with hasPermission(user, permission) function
  - [ ] Subtask 3.2: Implement union logic: collect all permissions from all assigned roles
  - [ ] Subtask 3.3: Handle wildcard permission ('\*') for System Administrator
  - [ ] Subtask 3.4: Implement permission matching logic (exact match and wildcard patterns like 'finance:\*')
  - [ ] Subtask 3.5: Add unit tests for permission checking (single role, multi-role, wildcard)
  - [ ] Subtask 3.6: Create usePermissions() composable wrapping permissions.js for Vue components
  - [ ] Subtask 3.7: Verify union logic works correctly for users with multiple roles

- [ ] **Task 4: Implement Route Guards** (AC: 5)
  - [ ] Subtask 4.1: Create src/boot/router-guards.js boot file
  - [ ] Subtask 4.2: Add beforeEach navigation guard to check route meta.requiresPermission
  - [ ] Subtask 4.3: Redirect unauthorized users to /unauthorized page (create page)
  - [ ] Subtask 4.4: Update routes.js to add requiresPermission meta to protected routes
  - [ ] Subtask 4.5: Test route guard with different user roles (should block/allow appropriately)
  - [ ] Subtask 4.6: Add router-guards to quasar.config.js boot files array

- [ ] **Task 5: Implement UI Conditional Rendering** (AC: 6)
  - [ ] Subtask 5.1: Update MainLayout.vue to conditionally show navigation items based on permissions
  - [ ] Subtask 5.2: Add v-if="hasPermission('residents:read')" to Residents menu item
  - [ ] Subtask 5.3: Add v-if="hasPermission('finance:read')" to Finance menu item
  - [ ] Subtask 5.4: Create PermissionGuard.vue component for reusable permission checks
  - [ ] Subtask 5.5: Test UI rendering with different user roles (menu items appear/disappear)

- [ ] **Task 6: Configure Appwrite Table Permissions** (AC: 7)
  - [ ] Subtask 6.1: Review current table permissions in Appwrite console
  - [ ] Subtask 6.2: Set residents table: read (authenticated), write (role:admin, role:village_head)
  - [ ] Subtask 6.3: Set households table: read (authenticated), write (role:admin, role:village_head)
  - [ ] Subtask 6.4: Set roles table: read (authenticated), write (role:admin)
  - [ ] Subtask 6.5: Test table permissions by attempting unauthorized operations
  - [ ] Subtask 6.6: Document permission model in README.md

- [ ] **Task 7: Create Admin Interface for Role Display** (AC: 8)
  - [ ] Subtask 7.1: Create src/pages/admin/UsersPage.vue (or update existing)
  - [ ] Subtask 7.2: Display list of all users with their assigned roles
  - [ ] Subtask 7.3: Show role names (not just IDs) by joining with roles table
  - [ ] Subtask 7.4: Add "Roles" column to users table showing comma-separated role names
  - [ ] Subtask 7.5: Add note: "Role editing coming in Epic 2"
  - [ ] Subtask 7.6: Restrict page access to System Administrator only
