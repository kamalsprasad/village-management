  ...
  <q-card class="full-height">
    <q-card-section class="row items-center justify-between">
      ...
      <div class="row q-gutter-sm">
        <q-btn v-if="!readOnly" outline color="primary" label="Record" icon="add" @click="showTransactionModal = true" />
      ...
      <q-list separator>
        ...
        >
          <q-item-section avatar>
            <q-avatar :color="transaction.type === 'income' ? 'positive' : 'negative'" text-color="white" size="sm">
              <q-icon :name="transaction.type === 'income' ? 'arrow_downward' : 'arrow_upward'" />
            </q-avatar>
          </q-item-section>
          
          <q-item-section>
            <q-item-label class="text-weight-medium">{{ getCategoryName(transaction.category_id) }}</q-item-label>
            <q-item-label caption lines="1">{{ formatReportDate(transaction.date) }} • {{ transaction.source_module }}</q-item-label>
          </q-item-section>
          
          <q-item-section side>
            <q-item-label class="text-weight-bold" :class="transaction.type === 'income' ? 'text-positive' : 'text-negative'">
              {{ transaction.type === 'income' ? '+' : '-' }}{{ formatCurrency(transaction.amount_funded) }}
            </q-item-label>
    ...
    <q-card-section v-else-if="!loading" class="text-center text-grey-6 q-pa-xl">
      ...
      <q-btn v-if="!readOnly" color="primary" outline label="Record First Transaction" class="q-mt-md" @click="showTransactionModal = true" />
    </q-card-section>
...
const onTransactionSaved = () => {
  ...
  // Note: App relies on financeStore reactive update or useDashboardData's debouncedRefresh
};
</script>
